...
document.addEventListener('DOMContentLoaded', function () {
    ...
    addToCartButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (quantity > 0) {
                ...
                if (item === 'Boiled Rice') {
                    const otherItemsInCart = cart.some(cartItem => cartItem.item !== 'Boiled Rice');
                    if (!otherItemsInCart) {
                        const servingToast = new bootstrap.Toast(document.getElementById('servingToast11'));
                ...
                if (['Chicken Biryani Seeraga Samba', 'Chicken Briyani Basmati Rice', 'Mutton Seeraga Samba', 'Mutton Basmati Rice'].includes(item)) {
                    const halwaItem = 'Extra Bread Halwa';
                    const halwaPrice = 15; // Set the price for Bread Halwa
                    const existingHalwaItem = cart.find(cartItem => cartItem.item === halwaItem);
                ...
                if (item === 'Curry Combo') {
                    const servingToast = new bootstrap.Toast(document.getElementById('servingToast1'));
    ...
    function updateCart(item, quantity) {
        ...
        if (cartItem) {
            ...
            if (quantity > 0) {
                const price = parseFloat(document.querySelector(`[data-item="${item}"]`).getAttribute('data-price'));
                ...
                <li class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <div class="d-flex align-items-center">
                        <div class="quantity-control me-3">
                            <button class="btn btn-sm btn-outline-secondary minus" data-item="${cartItem.item}">-</button>
                            <span class="mx-1 quantity">${cartItem.quantity}</span>
                            <button class="btn btn-sm btn-outline-secondary plus" data-item="${cartItem.item}">+</button>
                        </div>
    ...
    offcanvasCartSection.addEventListener('click', (event) => {
        if (event.target.classList.contains('minus') || event.target.classList.contains('plus')) {
            ...
            const quantityDisplay = document.querySelector(`[data-item="${item}"]`).parentElement.querySelector('.quantity');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Your existing cart functionality goes here...
    // (I'm assuming you've already added all the previous code here.)

    // Catering Booking Form submit event
    document.getElementById('cateringBookingForm').addEventListener('submit', function (event) {
        ...
        if (!isLoggedIn) {
            ...
            // Proceed with form submission or booking logic
            alert('Catering booking submitted successfully!');
        }
    });
});
...
document.addEventListener('DOMContentLoaded', function () {
    // Ensure the login button is available before adding the event listener
    ...
    if (loginBtn) {
        loginBtn.addEventListener('click', function () {
            ...
            // Basic validation (add more checks as necessary)
            if (name && password) {
                // Simulate linking to a database (you will replace this with your actual API call)
                setTimeout(function () {
                    if (successMsg) {
    ...
    if (togglePassword) {
        togglePassword.addEventListener('click', function () {
            ...
            // Toggle the eye icon class between 'bi-eye' and 'bi-eye-slash'
            if (type === 'password') {
...
// Wait for the DOM to fully load before running the script
document.addEventListener('DOMContentLoaded', function () {
    // Ensure the signup button is available before adding the event listener
            ...
            if (name && phone && email && password) {
                ...
                setTimeout(function () {
                    if (successMsg) {
                        ...
                        // Reset both forms after success
                        document.getElementById('signupDetailsForm').reset(); // Reset details form
                        document.getElementById('signupPasswordForm').reset(); // Reset password form

                        // Optionally hide the modal after success
                        // $('#signupModal').modal('hide'); // Uncomment if using jQuery to hide the modal
                    } else {
                        console.error('Success message element not found.');
                    }
                }, 1000); // Simulate API delay
            } else {
                alert('Please fill out all fields in both sections.');
            }
        });
    } else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Backend endpoint URL (replace with your actual backend URL)
    const backendUrl = 'https://167e-2405-201-e025-f04e-8199-2c38-813e-ed0d.ngrok-free.app';
        ...
        signupBtn.addEventListener('click', function () {
            ...
            fetch(backendUrl, {
                ...
                .then(data => {
                    ...
                    document.getElementById('signupPasswordForm').reset();
...
// Show eye icon when typing in login-in password field
password.addEventListener('input', function () {
    if (password.value.length > 0) {
        togglePassword.style.display = 'block';  // Show eye icon when there's input
    } else {
        togglePassword.style.display = 'none';   // Hide eye icon if input is cleared
    }
});
...
// Initially hide the eye icon
toggleSignInPassword.style.display = 'none';

// Show eye icon when typing in sign-in password field
signInPassword.addEventListener('input', function () {
    if (signInPassword.value.length > 0) {
        toggleSignInPassword.style.display = 'block';  // Show eye icon when there's input
    } else {
        toggleSignInPassword.style.display = 'none';   // Hide eye icon if input is cleared
    }
});
...
toggleSignInPassword.addEventListener('click', function () {
 ...
 function handleLogin(event) {
	...
	// Here you can add logic to check if the account exists
	// For now, we'll just simulate an account check
	const accountExists = false; // Change this based on your actual logic

	if (!accountExists) {
		alert("You don't have an account. Please sign in.");
	} else {
		// Logic for successful login (not implemented in this example)
...
document.addEventListener('DOMContentLoaded', function () {
    var loginStepper = new Stepper(document.querySelector('#loginStepper'));
  });
